  ...
  [true, false].forEach(lowResourceAnimation => {
    test(`should capture keypoints of the animation ${lowResourceAnimation ? '[low resource mode]' : ''}`, async ({
      ...
      for (let i = 0; i < testPoints.length; i++) {
        ...
        const seekPos = point.toString();
        const percentage = Math.round(point * 100);

        await page.setViewportSize({ width: frameWidth, height: frameHeight });
        await page.goto(
          `${STORYBOOK_BASE_URL}?id=animations-intro-animation--interactive-controls&args=seekPosition%3A${seekPos}%3BshowInfo%3Afalse%3BbackgroundImage%3Atrue%3BlowResourceAnimation%3A${lowResourceAnimation ? 'true' : 'false'}&viewMode=story`
        );
