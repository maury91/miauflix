...
const useCategories = () => {
  ...
  const { data: progressCategory, isLoading: isProgressCategoryLoading } = useGetProgressQuery(
    userId,
    {
      pollingInterval: 30000,
    }
  );
  const { data: normalCategories, isLoading: areNormalCategoriesLoading } = useGetCategoriesQuery();
  ...
  }, [areNormalCategoriesLoading, isProgressCategoryLoading, normalCategories, progressCategory]);
};
...
export const Categories: FC<CategoriesProps> = ({ onLeft, onMediaSelect, visible }) => {
    ...
    () =>
      on(['up', 'down'], direction => {
        ...
        if (isFocusedOnCategory) {
          ...
          const categoryIndex = categories.findIndex(category => category.id === focusedCategoryId);
          if (categoryIndex !== -1) {
  ...
  useEffect(() => {
    if (categories.length && firstRender) {
      const categoryToHighlight = categories.findIndex(({ id }) => id === highlightedCategoryId);
