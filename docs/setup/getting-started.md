...
The default Compose topology starts the standalone List Service. Configure the Trakt OAuth credentials and `LIST_SERVICE_ENCRYPTION_KEY` during setup, or disable the service if you do not use lists. The List Service key is not generated by the backend wizard.
