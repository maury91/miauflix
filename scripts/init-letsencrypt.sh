...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    *)
      ...
      print_status "Usage: $0 -d DOMAIN [-e EMAIL] [-w] [-s]"
      print_status "  -d, --domain DOMAIN  The domain to obtain certificates for"
      print_status "  -e, --email EMAIL    Email for Let's Encrypt registration and notifications"
      print_status "  -w, --www           Include www subdomain"
      print_status "  -s, --staging       Use Let's Encrypt staging server"
      exit 1
      ;;
  esac
done
...
print_status "=== Let's Encrypt Certificate Setup for ${domains[0]} ==="

# Ask for email if not set
if [ -z "$email" ]; then
  read -p "Enter your email (used for urgent renewal and security notices): " email
  if [ -z "$email" ]; then
    print_warning "Running without providing an email address is not recommended."
    print_warning "You will not receive important certificate expiration notifications."
...
if [ -d "$data_path/conf/live/${domains[0]}" ]; then
  print_warning "Existing certificate found for ${domains[0]}."
...
print_status "Setting proper permissions for certificate directories..."
chmod -R 755 "$data_path/conf" || print_warning "Could not set permissions on conf directory"
chmod -R 755 "$data_path/www" || print_warning "Could not set permissions on www directory"
...
if [ ! -e "$data_path/conf/options-ssl-nginx.conf" ] || [ ! -e "$data_path/conf/ssl-dhparams.pem" ]; then
  ...
  if [ $? -ne 0 ]; then
    print_error "Failed to download TLS parameters. Please check your internet connection."
    exit 1
  fi
...
print_status "Starting Nginx for certificate request..."
...
if [ "$HTTP_STATUS" != "200" ]; then
  print_error "Challenge path is not accessible (status code: $HTTP_STATUS)"
  print_warning "Please check your Nginx configuration and domain settings"
  exit 1
fi
print_status "✓ Challenge path is accessible!"
...
print_status "Requesting Let's Encrypt certificates for ${domains[0]}..."
...
print_status "Issuing Let's Encrypt certificate request. This will validate your domain ownership..."
print_warning "Note: This validation process can only succeed when:"
print_warning "1. Your domain actually points to this server's IP address"
print_warning "2. This server is accessible from the Internet on port 80"
print_warning "3. The domain validation server can reach your Nginx container"
echo ""

# Using Docker to obtain the certificate to avoid permission issues
print_status "Requesting certificate with certbot. This may take a few minutes..."
...
if [ $EXIT_CODE -ne 0 ]; then
  ...
  print_warning "- Domain doesn't point to this server's IP"
  print_warning "- Port 80 is not accessible from the Internet"
  print_warning "- Let's Encrypt rate limits (use --staging/-s flag for testing)"
  print_warning "- Nginx configuration issue with the .well-known/acme-challenge/ path"
  echo ""
  print_warning "For testing locally, you can use self-signed certificates instead."
  exit 1
else
  print_status "Certificate successfully obtained!"
fi

# Update environment variables with the certificate paths
print_status "Updating environment variables with certificate paths..."
...
if [ $staging -eq 1 ]; then
  print_warning "Note: You used the staging environment. The certificate won't be trusted by browsers."
  print_warning "When you're ready to get a production certificate, run this script again without the -s/--staging flag."
else
  print_status "Your site should now be accessible at https://${domains[0]}"
fi
